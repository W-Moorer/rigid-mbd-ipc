...
\bi
  ...
  \ei
  \item \mybold{Python parts} (this list is continuously extended, see \refSection{sec:pythonUtilityFunctions}):
  \bi
...
See also \refSection{sec:referenceAndCurrentCoordinates}!

Note that commonly the \texttt{OutputVariableType} \texttt{Coordinates} returns coordinates without reference values, which are usually displacements (or changes in the rotation parameters), which is required if you are interested, e.g., in motion of finite element nodes.
...
or triads given by rotation matrices. For further information, see the \texttt{sensors.traces} structure of \texttt{VisualizationSettings}, \refSection{sec:VSettingsTraces}.

\mysubsubsectionlabel{Renderer and 3D graphics}{sec:overview:basics:renderer}
A 3D renderer is attached to the simulation. Visualization is started with  \texttt{SC.renderer.Start()}, see the examples and tutorials.
...
\noindent Here are the \mybold{main features of the renderer}, using keyboard and mouse, for details see \refSection{sec:graphicsVisualization}:
\bi
  ...
  \item raytracing mode, see \refSection{sec:overview:basics:raytracing} 
\ei
%
Depending on your model (size, place, ...), you \mybold{may need to adjust the following general visualization} and \texttt{openGL} \mybold{parameters} in \texttt{visualizationSettings}, see \refSection{sec:VisualizationSettingsMain}:
\bi
...
Details on visualization settings and its substructures are provided in \refSection{sec:VisualizationSettingsMain}. These settings may also be edited by pressing 'V' in the active render window (does not work, if there is no active render loop using, e.g., \texttt{SC.renderer.DoIdleTasks()} ).
...
\mysubsubsectionlabel{Raytracing}{sec:overview:basics:raytracing}
...
\LatexRSTfigure{figures/raytracerDemo.jpg}{fig:raytracerDemo}{12cm}{400}{Example image of raytraced renderer view.}
...
\noindent \mybold{Materials}:
\bi
  \item Materials have the type \texttt{VSettingsMaterial}, see description in \refSection{sec:VSettingsMaterial}, for adjusting color, reflectivity, shininess, alpha-transparency, etc.; 
  ...
  \item System text messages (solver, version, etc.) are overlayed over raytracing and can be turned off using the settings in \texttt{general.showComputationInfo} and similar. However, note that \mybold{item texts are currently not shown} in raytracer, affecting node numbers, etc.!
\ei

\noindent \mybold{Limitations and risks}:
\bi
  ...
  \item In general, it is \mybold{recommended to start with default settings and experiment} with changes using the visualization settings dialog.
\ei
...
There is a simple way to store the current view (zoom, centerpoint, orientation, etc.) by using \texttt{SC.renderer.GetState()} and \texttt{SC.renderer.SetState()},
...
\end{lstlisting}
%
\mybold{By default and as a recommended way}, if no solution is provided, \texttt{SolutionViewer} tries to reload the solution of the previous simulation that is referred to from \texttt{mbs.sys['simulationSettings']}:
\pythonstyle\begin{lstlisting}
...
The easiest way to create animations, is using the SolutionViewer with its integrated features, see \refSection{sec:overview:basics:solutionviewer}.
...
\bi
  ...
  \bi
  \item[$\ra$] this may happen for \mybold{system without loads}; larger number of steps may happen for finer discretization; 
...
\bi
  ...
  \item Python user functions can be speed up (since \codeName V1.7.40) by converting conventional Python functions into \codeName (internal) symbolic user functions, which have similar performance as C++ functions with the ability to parallelize; see \refSection{sec:cinterface:symbolic}
...
This separate module \texttt{GeneralContact} \lbrack\mybold{still under development, consider with care!}\rbrack\ is highly optimized and implemented with parallelization (multi-threaded) for certain types of contact elements.
\onlyRST{
...
\noindent \mybold{Note}:
\bi
\item \texttt{GeneralContact} is (in most cases) restricted to dynamic simulation (explicit or implicit \lbrack\mybold{still under development, consider with care!}\rbrack ) if friction is used; without friction, it also works in the static case
...
\mybold{Exceptions} to the NO-ABBREVIATIONS-RULE\ignoreRST{, see also \hyperref[sec:listOfAbbreviations]{\underline{List of Abbreviations}}}: %no section number!!!
\bi
    \item \ac{ODE}
