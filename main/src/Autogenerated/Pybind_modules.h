...
// AUTO:  ++++++++++++++++++++++
    ...
    py::class_<GeneralizedAlphaSettings>(m, "GeneralizedAlphaSettings", "GeneralizedAlphaSettings class") // AUTO: 
        ...
        .def_readwrite("lieGroupSimplifiedKinematicRelations", &GeneralizedAlphaSettings::lieGroupSimplifiedKinematicRelations, "True: for Lie group nodes, the integrator uses the original kinematic relations of the Bruls and Cardona 2010 paper")
    ...
    py::class_<VSettingsRaytracer>(m, "VSettingsRaytracer", "VSettingsRaytracer class") // AUTO: 
        ...
        .def_readwrite("enable", &VSettingsRaytracer::enable, "True: use (software) raytracer; False: use standard OpenGL renderer")
        .def_property("globalFogColor", &VSettingsRaytracer::PyGetGlobalFogColor, &VSettingsRaytracer::PySetGlobalFogColor)
        .def_readwrite("globalFogDensity", &VSettingsRaytracer::globalFogDensity, "global fog density; fog is deactivated if fogDensity=0, otherwise it is a density relative to scene max size; as it is relative, the factor has to be relatively high to be visible (usually >1)")
        ...
        .def_readwrite("lightRadius", &VSettingsRaytracer::lightRadius, "if lightRadiusVariations>1, it uses the given radius for all lights, to convert point lights into distributed lights (slower)")
        ...
        .def_readwrite("verbose", &VSettingsRaytracer::verbose, "True: print out some debug information on rendering, in particular rendering timings and counter")
        .def_readwrite("zBiasLines", &VSettingsRaytracer::zBiasLines, "offset for lines to draw in front of faces; relative to scene radius")
        .def_readwrite("zOffsetCamera", &VSettingsRaytracer::zOffsetCamera, "offset for for camera towards the scene; use positive factor put camera inside, e.g. of brick (like a room) or sphere; use (slightly) negative value to make whole scene visible")
    ...
    py::class_<VSettingsOpenGL>(m, "VSettingsOpenGL", "VSettingsOpenGL class") // AUTO: 
        ...
        .def_readwrite("clippingPlaneDistance", &VSettingsOpenGL::clippingPlaneDistance, "distance of clipping plane on normal vector; see also clippingPlaneNormal")
        .def_property("clippingPlaneNormal", &VSettingsOpenGL::PyGetClippingPlaneNormal, &VSettingsOpenGL::PySetClippingPlaneNormal)
        .def_readwrite("depthSorting", &VSettingsOpenGL::depthSorting, "True (slower): sort triangles by Z-depth to remove transparency artifacts: only works if triangles do not intersect or come close (you may like to refine triangle meshes); False: no depth-sort (faster)")
        ...
        .def_readwrite("lightPositionsInCameraFrame", &VSettingsOpenGL::lightPositionsInCameraFrame, "set False to set light positions and directions relative to model frame; True: lights are in global (camera) frame; this is always False for raytracer; this was True up to Exudyn 1.9.174")
    ...
    py::class_<VSettingsInteractive>(m, "VSettingsInteractive", "VSettingsInteractive class") // AUTO: 
        ...
        .def_readwrite("selectionLeftMouseItemTypes", &VSettingsInteractive::selectionLeftMouseItemTypes, "binary flags (1,2,4,8,16) for (Node,Object,Marker,Load,Sensor) that are identified with left mouse click selection")
