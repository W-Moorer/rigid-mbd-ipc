...
public: //! objects made public, because Python functions are anyway mostly Autogenerated ==> keep interface simple
	CSystem cSystem;			//!< CSystem object; cSystem must always be initialized correctly!
...
public:
	...
	bool HasMainSystemContainer() const { return mainSystemContainerBacklink != nullptr; }
	...
	//! get user function 
	py::object PyGetPreStepUserFunction(bool asDict = false);

	//! set user function to be called by solvers at end of step, just before writing results (static or dynamic step)
	void PySetPostStepUserFunction(const py::object& value);
	//! get user function 
	py::object PyGetPostStepUserFunction(bool asDict = false);
