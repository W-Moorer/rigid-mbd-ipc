...
void VisualizationObjectANCFCable::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	//std::array<Vector3D, 4> points; //quad points
	//std::array<Float4, 4> colors;   //quad colors
	...
	for (Index i = 1; i <= tiling; i++)
	{
		...
		for (Index j = 0; j < tilingCyl; j++)
		{
			...
			else //draw cylinder hull
			{
				//if (false)
				//{
				//	colors[0] = item.color1;
				//	colors[1] = item.color1;
				//	colors[2] = item.color2;
				//	colors[3] = item.color2;

				//	points[0] = p0 + radius * crossVecA0;
				//	points[1] = p0 + radius * crossVecB0;
				//	points[2] = p1 + radius * crossVecB1;
				//	points[3] = p1 + radius * crossVecA1;

				//	vSystem->graphicsData.AddQuad(points, colors, itemID);
				//}
				//else
				{
...
void VisualizationMarkerBodiesRelativeTranslationCoordinate::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	EXUvis::DrawMarker(pos0, radius, currentColor, vSystem->graphicsData, itemID, !visualizationSettings.markers.drawSimplified && visualizationSettings.openGL.showFaces);
	EXUvis::DrawMarker(pos1, radius, currentColor, vSystem->graphicsData, itemID, !visualizationSettings.markers.drawSimplified && visualizationSettings.openGL.showFaces);
...
void VisualizationLoadMassProportional::UpdateGraphics(const VisualizationSettings& visualizationSettings, VisualizationSystem* vSystem, Index itemNumber)
{
	...
	if (cMarker->GetType() & Marker::BodyMass) //marker must be able to measure position
	{
		...
		//	Vector3D n2;
		//	EXUmath::ComputeOrthogonalBasisVectors(loadVector, n1, n2);
