...
#include "Autogenerated/MainMarkerBodiesRelativeTranslationCoordinate.h"
#include "Autogenerated/MainMarkerBodiesRelativeRotationCoordinate.h"
...
#include "Autogenerated/MainObjectContactSphereTorus.h"
#include "Autogenerated/MainObjectContactSphereTriangle.h"
...
bool MainMarkerBodiesRelativeTranslationCoordinate::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CMarkerBodiesRelativeTranslationCoordinate* cMarker = (CMarkerBodiesRelativeTranslationCoordinate*)GetCMarker();

	//objects are already checked to be body type in CSystem
	if (fabs(cMarker->GetParameters().axis0.GetL2Norm() - 1.) > toleranceNorm)
	{
		errorString = "MarkerBodiesRelativeTranslationCoordinate: axis0 must have length 1";
		return false;
	}
	return true;
}

bool MainMarkerBodiesRelativeRotationCoordinate::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CMarkerBodiesRelativeRotationCoordinate* cMarker = (CMarkerBodiesRelativeRotationCoordinate*)GetCMarker();

	//objects are already checked to be body type in CSystem
	if (fabs(cMarker->GetParameters().axis0.GetL2Norm() - 1.) > toleranceNorm)
	{
		errorString = "MainMarkerBodiesRelativeRotationCoordinate: axis0 must have length 1";
		return false;
	}
	return true;
}
...
bool MainMarkerSuperElementPosition::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	if (fabs(sumWF - 1.) > toleranceChecks)
	{
...
bool MainMarkerKinematicTreeRigid::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	const CMarkerKinematicTreeRigid* cMarker = GetCMarkerKinematicTreeRigid();

	//valid object number has been already checked in CSystem::CheckSystemIntegrity
	Index objectNumber = cMarker->GetObjectNumber();
	const CObject* cObjectPtr = mainSystem.GetMainSystemData().GetMainObjects()[objectNumber]->GetCObject();

	if (((Index)cObjectPtr->GetType() & (Index)CObjectType::KinematicTree) == 0)
	{
		errorString = "MarkerKinematicTreeRigid: can only be applied to a object of type 'KinematicTree'";
		return false;
	}

	const CObjectKinematicTree& cObjectKinematicTree = (const CObjectKinematicTree&)(*cObjectPtr); //now possible
	if (cMarker->GetParameters().linkNumber < 0 || cMarker->GetParameters().linkNumber >= cObjectKinematicTree.NumberOfLinks())
	{
		errorString = "MarkerKinematicTreeRigid: link number must be within range of KinematicTree (0 .." + 
			EXUstd::ToString(cObjectKinematicTree.NumberOfLinks()) + 
			"), but received linkNumber="+ EXUstd::ToString(cMarker->GetParameters().linkNumber);
		return false;
	}
...
bool MainNodeRigidBodyEP::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	if (fabs(ep.GetL2Norm() - 1.) > toleranceNorm)
	{
...
//OBJECTS
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
bool MainObjectANCFCable::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectANCFCable* cObject = (CObjectANCFCable*)GetCObject();
	
	for (Index i = 0; i < 2; i++) {
		if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(cObject->GetNodeNumber(i)).GetTypeName(), "PointSlope1") != 0)
		{
			errorString = "ObjectANCFCable2D: node " + EXUstd::ToString(i) + " must be of type 'PointSlope1', but found type '" +
				mainSystem.GetMainSystemData().GetMainNode(cObject->GetNodeNumber(i)).GetTypeName() + "'";
			return false;
		}
	}
	if (cObject->GetNumberOfNodes() != 2)
	{
		errorString = "ObjectANCFCable: must have 2 nodes but received " + 
			EXUstd::ToString(cObject->GetNumberOfNodes()) + " nodes.";
		return false;
...
bool MainObjectConnectorLinearSpringDamper::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
    CObjectConnectorLinearSpringDamper* cObject = (CObjectConnectorLinearSpringDamper*)GetCObject();

    if (fabs(cObject->GetParameters().axisMarker0.GetL2Norm() - 1) > toleranceNorm)
    {
        errorString = "CObjectConnectorLinearSpringDamper: axisMarker0 must be normalized (length must be 1)";
        return false;
    }
...
bool MainObjectConnectorCoordinateSpringDamperExt::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
    ...
    {
        STDstring str = "CObjectConnectorCoordinateSpringDamperExt: because of ";
        if (cObject->GetParameters().useLimitStops) { str += "useLimitStops is True, "; }
    ...
    if (cObject->GetParameters().frictionProportionalZone == 0 || cObject->GetParameters().useLimitStops)
    {
	...
	if (cObject->GetParameters().factor0 == 0)
	{
		errorString = "CObjectConnectorCoordinateSpringDamperExt: factor0 may not be zero";
		return false;
	}

	if (cObject->GetParameters().factor1 == 0)
	{
		errorString = "CObjectConnectorCoordinateSpringDamperExt: factor1 may not be zero";
		return false;
	}
...
bool MainObjectConnectorHydraulicActuatorSimple::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectConnectorHydraulicActuatorSimple* cObject = (CObjectConnectorHydraulicActuatorSimple*)GetCObject();

	Index nn = cObject->GetNumberOfNodes();
	if (nn != 1)
	{
		errorString = "CObjectConnectorHydraulicActuatorSimple: number of nodes must be 1";
		return false;
	}

	CNode* node = cObject->GetCNode(0);
	if (!EXUstd::IsOfType(node->GetType(), Node::GenericODE1))
	{
		errorString = "CObjectConnectorHydraulicActuatorSimple: node 0 must be a GenericODE1 node";
		return false;
	}

	if (node->GetNumberOfODE1Coordinates() != 2)
	{
		errorString = "CObjectConnectorHydraulicActuatorSimple: GenericODE1 node 0 must have 2 ODE1 coordinates";
		return false;
	}

	if (node->GetReferenceCoordinateVector().GetL2NormSquared() != 0.)
	{
		errorString = "CObjectConnectorHydraulicActuatorSimple: GenericODE1 node 0 must have zero reference values";
		return false;
	}
...
bool MainObjectConnectorReevingSystemSprings::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectConnectorReevingSystemSprings* cObject = (CObjectConnectorReevingSystemSprings*)GetCObject();

	Index hasCoordinateMarkers = (Index)cObject->GetParameters().hasCoordinateMarkers;
	Index nRigidBodyMarkers = cObject->GetParameters().markerNumbers.NumberOfItems() - 2 * hasCoordinateMarkers;

	if (nRigidBodyMarkers < 2)
	{
		if (hasCoordinateMarkers)
		{
			errorString = "CObjectConnectorReevingSystemSprings: has " + EXUstd::ToString(nRigidBodyMarkers) + 
				" markers, but the minimum is 4 (2 rigid body markers and 2 coordinate markers)";
		}
		else
		{
			errorString = "CObjectConnectorReevingSystemSprings: has " + EXUstd::ToString(nRigidBodyMarkers) +
				" rigid body markers, but the minimum is 2";
		}
		return false;

	}

	Index npa = cObject->GetParameters().sheavesAxes.NumberOfItems();
	if (nRigidBodyMarkers != npa)
	{
		errorString = "CObjectConnectorReevingSystemSprings: number of rigid body markers is " + EXUstd::ToString(nRigidBodyMarkers);
		if (hasCoordinateMarkers) { errorString += " (note that two additional markers are expected to be coordinate markers)"; }
		errorString += ", but the number of sheavesAxes is different (" + EXUstd::ToString(npa) + ")";
		return false;
	}

	for (Index i = 0; i < npa; i++)
	{
		Vector3D v = cObject->GetParameters().sheavesAxes[i];
		if (fabs(v.GetL2Norm() - 1.) > toleranceNorm)
		{
			errorString = "CObjectConnectorReevingSystemSprings: axis " + EXUstd::ToString(i) +
				" in sheavesAxes has length which is considerably different from 1, which is not possible";
			return false;
		}
	}
	...
	if (nRigidBodyMarkers != npr)
	{
		errorString = "CObjectConnectorReevingSystemSprings: number of rigid body markers is " + EXUstd::ToString(nRigidBodyMarkers);
		if (hasCoordinateMarkers) { errorString += " (note that two additional markers are expected to be coordinate markers)"; }
		errorString += ", but the number of sheavesRadii is different (" + EXUstd::ToString(npr) + ")";
		return false;
	}
	...
	for (Index i = 0; i < nRigidBodyMarkers; i++)
	{
		...
		{
			errorString = "CObjectConnectorReevingSystemSprings: marker " + EXUstd::ToString(i) + " must be a rigid body marker";
			return false;
		}
	}
	...
	for (Index i = 0; i < 2* hasCoordinateMarkers; i++)
	{
		...
		{
			errorString = "CObjectConnectorReevingSystemSprings: marker " + EXUstd::ToString(i) + " must be a coordinate marker; because hasCoordinateMarkers=True, the last two markers in markerNumbers are expected to provide coordinates";
			return false;
		}
	}
...
bool MainObjectConnectorRollingDiscPenalty::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	//	return false;
	//}
	if ((cObject->GetParameters().planeNormal.GetL2Norm() - 1.) > toleranceNorm)
	{
...
bool MainObjectContactSphereSphere::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectContactSphereSphere* cObject = (CObjectContactSphereSphere*)GetCObject();
	Index node = cObject->GetNodeNumber(0);
	...
	{
		errorString = "CObjectContactSphereSphere: node number is invalid!";
		return false;
	}
	...
	if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(node).GetTypeName(), "GenericData") != 0)
	{
		errorString = "CObjectContactSphereSphere: node must be of type 'GenericData'";
		return false;
	}
	...
	{
		errorString = STDstring("CObjectContactSphereSphere: NodeGenericData (Node ") + 
			EXUstd::ToString(node) + ") must have " + EXUstd::ToString(cObject->GetDataVariablesSize())
			+ " coordinates (found: " + EXUstd::ToString(nc) + ")";
		return false;
	}


	Index requestedMarkerType = (Index)(Marker::Position);
	...
	{
		errorString = STDstring("CObjectContactSphereSphere: Marker 0 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
		return false;
	}

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactSphereSphere: Marker 1 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
		return false;
	}
	
	if (!IsInRange(errorString, cObject->GetParameters().restitutionCoefficient, 0., 1.,
		"CObjectContactSphereSphere::CheckPreAssembleConsistency: ", "restitutionCoefficient")) { return false; }

	if (!IsInRange(errorString, cObject->GetParameters().impactModel, 0, 2,
		"CObjectContactSphereSphere::CheckPreAssembleConsistency: ", "impactModel")) { return false; }

	if (!IsInRange(errorString, cObject->GetParameters().contactPlasticityRatio, 0., 1. - 2*EXUstd::EPSILONREAL,
		"CObjectContactSphereSphere::CheckPreAssembleConsistency: ", "contactPlasticityRatio")) {
		return false;
	}

	if (cObject->GetParameters().spheresRadii[0] <= 0. ||
		cObject->GetParameters().spheresRadii[1] <= 0.)
	{
		errorString = STDstring("CObjectContactSphereSphere::CheckPreAssembleConsistency: each radius in spheresRadii must be greater than zero");
		return false;
	}
...
bool MainObjectContactSphereTorus::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectContactSphereTorus* cObject = (CObjectContactSphereTorus*)GetCObject();
	Index node = cObject->GetNodeNumber(0);

	if (cObject->GetParameters().nodeNumber == EXUstd::InvalidIndex ||
		cObject->GetParameters().nodeNumber >= mainSystem.GetMainSystemData().GetMainNodes().NumberOfItems())
	{
		errorString = "CObjectContactSphereTorus: node number is invalid!";
		return false;
	}
	...
	if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(node).GetTypeName(), "GenericData") != 0)
	{
		errorString = "CObjectContactSphereTorus: node must be of type 'GenericData'";
		return false;
	}
	...
	if (nc != cObject->GetDataVariablesSize())
	{
		errorString = STDstring("CObjectContactSphereTorus: NodeGenericData (Node ") +
			EXUstd::ToString(node) + ") must have " + EXUstd::ToString(cObject->GetDataVariablesSize())
			+ " coordinates (found: " + EXUstd::ToString(nc) + ")";
		return false;
	}
	...
	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[0]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactSphereTorus: Marker 0 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
		return false;
	}

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactSphereTorus: Marker 1 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
		return false;
	}

	if (!IsInRange(errorString, cObject->GetParameters().restitutionCoefficient, 0., 1.,
		"CObjectContactSphereTorus::CheckPreAssembleConsistency: ", "restitutionCoefficient")) {
		return false;
	}

	if (!IsInRange(errorString, cObject->GetParameters().impactModel, 0, 2,
		"CObjectContactSphereTorus::CheckPreAssembleConsistency: ", "impactModel")) {
		return false;
	}
	...
	//	)
	//{
	//	errorString = STDstring("CObjectContactSphereTorus::CheckPreAssembleConsistency: sphere radius must not be greater than torusMinorRadius");
	//	return false;
	//}

	if (fabs(cObject->GetParameters().torusAxis.GetL2Norm()-1) > toleranceNorm)
	{
		errorString = STDstring("CObjectContactSphereTorus::CheckPreAssembleConsistency: torus axis must have norm 1 (unit vector)");
		return false;
	}
	return true;
}

bool MainObjectContactSphereTriangle::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	CObjectContactSphereTriangle* cObject = (CObjectContactSphereTriangle*)GetCObject();
	Index node = cObject->GetNodeNumber(0);

	if (cObject->GetParameters().nodeNumber == EXUstd::InvalidIndex ||
		cObject->GetParameters().nodeNumber >= mainSystem.GetMainSystemData().GetMainNodes().NumberOfItems())
	{
		errorString = "CObjectContactSphereTriangle: node number is invalid!";
		return false;
	}
	...
	if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(node).GetTypeName(), "GenericData") != 0)
	{
		errorString = "CObjectContactSphereTriangle: node must be of type 'GenericData'";
		return false;
	}
	...
	if (nc != cObject->GetDataVariablesSize())
	{
		errorString = STDstring("CObjectContactSphereTriangle: NodeGenericData (Node ") +
			EXUstd::ToString(node) + ") must have " + EXUstd::ToString(cObject->GetDataVariablesSize())
			+ " coordinates (found: " + EXUstd::ToString(nc) + ")";
		return false;
	}
	...
	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[0]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactSphereTriangle: Marker 0 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
		return false;
	}

	if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[1]).GetType(), requestedMarkerType))
	{
		errorString = STDstring("CObjectContactSphereTriangle: Marker 1 must generally be of type = 'Rigid' (Position + Orientation) or of type Position, if friction is zero");
		return false;
	}

	if (!IsInRange(errorString, cObject->GetParameters().restitutionCoefficient, 0., 1.,
		"CObjectContactSphereTriangle::CheckPreAssembleConsistency: ", "restitutionCoefficient")) {
		return false;
	}

	if (!IsInRange(errorString, cObject->GetParameters().impactModel, 0, 2,
		"CObjectContactSphereTriangle::CheckPreAssembleConsistency: ", "impactModel")) {
		return false;
	}
...
bool MainObjectContactCurveCircles::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	if (cObject->GetParameters().nodeNumber == EXUstd::InvalidIndex ||
		cObject->GetParameters().nodeNumber >= mainSystem.GetMainSystemData().GetMainNodes().NumberOfItems())
	{
		errorString = "CObjectContactCurveCircles: node number is invalid!";
		return false;
	}
	...
	if (std::strcmp(mainSystem.GetMainSystemData().GetMainNode(node).GetTypeName(), "GenericData") != 0)
	{
		errorString = "CObjectContactCurveCircles: node must be of type 'GenericData'";
		return false;
	}
	...
	if (nc != cObject->GetDataVariablesSize())
	{
		errorString = STDstring("CObjectContactCurveCircles: NodeGenericData (Node ") +
			EXUstd::ToString(node) + ") must have " + EXUstd::ToString(cObject->GetDataVariablesSize())
			+ " coordinates, being 3 coordinates per contact circle; (found: " + EXUstd::ToString(nc) + ")";
		return false;
	}
	...
	for (Index i = 1; i < nMarkers.NumberOfItems(); i++)
	{
		if (!EXUstd::IsOfTypeAndNotNone((Index)mainSystem.GetCSystem().GetSystemData().GetCMarker(nMarkers[i]).GetType(), Marker::Position + Marker::Orientation))
		{
			errorString = STDstring("CObjectContactCurveCircles: Circle Marker ")+EXUstd::ToString(i)+" must be of type = 'Rigid' (Position + Orientation";
			return false;
		}
	}

	if (!cObject->GetParameters().polynomialData.UseDenseMatrix() || !cObject->GetParameters().segmentsData.UseDenseMatrix())
	{
		errorString = STDstring("CObjectContactCurveCircles: polynomialData and segmentsData must both use the DenseMatrix format");
		return false;
	}


	
	if (cObject->GetParameters().polynomialData.NumberOfColumns() != 0)
	{
		errorString = STDstring("CObjectContactCurveCircles: polynomialData must have zero size, as it is not implemented yet");
		return false;
	}

	//add check for polynomialData having same rows as cObject->GetNumberOfSegments()

	//segment data structure (rows is used to define the number of segments)
	if (cObject->GetParameters().segmentsData.NumberOfColumns() != 4)
	{
		errorString = STDstring("CObjectContactCurveCircles: segmentsData must have 4 columns, containing (x0,y0,x1,y1) coordinates, where 0 is the first segment point and 1 is the second point");
		return false;
	}

	if (cObject->GetParameters().segmentsData.NumberOfRows() == 0)
	{
		errorString = STDstring("CObjectContactCurveCircles: segmentsData must have at least 1 row");
		return false;
	}

	if (cObject->GetParameters().circlesRadii.NumberOfItems() != cObject->GetNumberOfCircles())
	{
		errorString = STDstring("CObjectContactCurveCircles: circlesRadii must have dimension of number of markers-1 (or in other words: the size of markerNumbers must be the number of circles + 1)");
		return false;
	}
...
bool MainObjectKinematicTree::CheckPreAssembleConsistency(const MainSystem& mainSystem, STDstring& errorString) const
{
	...
	Index cnt = 0;
	for (auto item : cObject->GetParameters().jointTypes)
	{
		if (!Joint::IsValid(item))
		{
			errorString = "ObjectKinematicTree: linkType of link number " + EXUstd::ToString(cnt) + " is invalid!";
			return false;
		}
		cnt++;
	}
